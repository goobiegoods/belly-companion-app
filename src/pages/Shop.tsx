  ...
  if (selectedCourse && selectedLesson !== null) {
    ...
    const handleComplete = async () => {
      ...
      if (isLast) {
        ...
        setReflectionText(""); setReflectionSaved(false); setQuizAnswer(null); setQuizSubmitted(false);
      }
    };

    return (
      <div className="min-h-screen flex flex-col page-enter" style={{ background: "#FFF8F5" }}>
        ...
        <div className="px-5 pt-5 pb-6 relative belly-hero-gradient">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 pb-28 space-y-5">
          ...
          <div className="rounded-[14px] p-4" style={{ background: "#FFFFFF", border: "1px solid #FFE4D4" }}>
            ...
            <textarea value={reflectionText} onChange={e => { setReflectionText(e.target.value); setReflectionSaved(false); }}
              placeholder="Write your thoughts..." className="w-full rounded-[10px] p-3 text-[13px] resize-none min-h-[80px] font-display italic"
            ...
            <button disabled={!reflectionText.trim() || reflectionSaved}
              ...
              className="mt-2 rounded-[12px] px-5 py-2.5 text-[13px] font-semibold disabled:opacity-50" style={{ background: "#FFB899", color: "#2A1200" }}>
              Save my reflection 💭
            </button>
            {reflectionSaved && <p className="text-[12px] mt-1.5" style={{ color: "#C8E6C0" }}>Saved 🌸</p>}
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {lesson.quiz.options.map((opt, i) => {
                ...
                let borderColor = "#FFE4D4", bg = "white";
                ...
                return (
                  <button key={i} disabled={quizSubmitted}
                    onClick={() => { setQuizAnswer(i); setQuizSubmitted(true); }}
        ...
        <div className="fixed bottom-0 left-0 right-0 flex items-center gap-3 px-5 py-3 bg-white" style={{ borderTop: "1px solid #FFE4D4" }}>
          {selectedLesson > 0 && (
            <button onClick={() => { setSelectedLesson(selectedLesson - 1); setReflectionText(""); setReflectionSaved(false); setQuizAnswer(null); setQuizSubmitted(false); }}
              className="h-11 px-4 rounded-[12px] text-[13px] font-semibold" style={{ background: "#FFF0E8", color: "#D4906A" }}>← Previous</button>
          )}
          <button onClick={handleComplete} disabled={isCompleted}
            className="flex-1 h-11 rounded-[12px] text-[14px] font-bold disabled:opacity-50" style={{ background: "#FFB899", color: "#2A1200" }}>
          ...
          {lessons.map((lesson, i) => {
            ...
            return (
              <button key={lesson.id} onClick={() => { setSelectedLesson(i); setReflectionText(""); setQuizAnswer(null); setQuizSubmitted(false); }}
                className="w-full rounded-[18px] p-4 flex items-center gap-3 text-left belly-press belly-card-shadow"
    ...
    <div className="min-h-screen pb-20 page-enter" style={{ background: "#FFF8F5" }}>
      ...
      <div className="flex gap-2 px-5 my-3">
        {(["remedies", "learn"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            style={{
              background: tab === t ? "#FFB899" : "#FFF0E8",
              color: tab === t ? "#2A1200" : "#D4906A",
      ...
      {tab === "remedies" ? (
        <>
          ...
          <div className="flex gap-3 px-5 overflow-x-auto hide-scrollbar mb-5">
            {kits.map(kit => (
              <div key={kit.id} className="min-w-[200px] rounded-[18px] p-[14px] belly-card-shadow shrink-0" style={{ background: "white", border: "1px solid #FFE4D4" }}>
                <div className="w-[32px] h-[32px] rounded-full flex items-center justify-center text-[18px] mb-2" style={{ background: "#FFF0E8" }}>{kit.emoji}</div>
                {kit.tag && <span className="text-[9px] px-2 py-0.5 rounded-full font-semibold belly-badge-glass mb-1 inline-block" style={{ background: "#FFF0E8", color: "#D4906A" }}>{kit.tag}</span>}
                ...
                <button onClick={() => addToCart(kit)} className="w-full mt-2 rounded-[20px] py-[6px] text-[11px] font-bold belly-btn-press" style={{ background: "#FFB899", color: "#2A1200" }}>Add to cart →</button>
              </div>
            ))}
          </div>
          ...
          <div className="px-5 space-y-2 mb-5">
            {remedies.map(rem => (
              <div key={rem.id} className="rounded-[16px] p-[13px_15px] flex items-center gap-3 belly-card-shadow" style={{ background: "white", border: "1px solid #FFE4D4" }}>
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-[18px] shrink-0" style={{ background: "#FFF0E8" }}>{rem.emoji}</div>
                <div className="flex-1 min-w-0">
                  ...
                  {rem.tag && <span className="text-[9px] px-2 py-0.5 rounded-full inline-block mt-0.5" style={{ background: "#FFF0E8", color: "#D4906A" }}>{rem.tag}</span>}
                </div>
                <div className="text-right shrink-0">
                  ...
                  <button onClick={() => addToCart(rem)} className="mt-1 rounded-[16px] px-3 py-1 text-[10px] font-bold belly-btn-press" style={{ background: "#FFB899", color: "#2A1200" }}>Add</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="px-5 space-y-2 mb-5">
            {teas.map(tea => (
              <div key={tea.id} className="rounded-[16px] p-[13px_15px] flex items-center gap-3 belly-card-shadow" style={{ background: "white", border: "1px solid #FFE4D4" }}>
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-[18px] shrink-0" style={{ background: "#FFF0E8" }}>{tea.emoji}</div>
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-semibold" style={{ color: "#2A1200" }}>{tea.name}</p>
                  <p className="text-[11px]" style={{ color: "#D4906A" }}>{tea.use}</p>
                  {tea.tag && <span className="text-[9px] px-2 py-0.5 rounded-full inline-block mt-0.5" style={{ background: "#FFF0E8", color: "#D4906A" }}>{tea.tag}</span>}
                </div>
                <div className="text-right shrink-0">
                  ...
                  <button onClick={() => addToCart(tea)} className="mt-1 rounded-[16px] px-3 py-1 text-[10px] font-bold belly-btn-press" style={{ background: "#FFB899", color: "#2A1200" }}>Add</button>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Disclaimer */}
          <div className="mx-5 mb-5 rounded-[12px] p-3" style={{ background: "#FFF4EE" }}>
            ...
            {homeopathyCourses.map(course => {
              ...
              return (
                <button key={course.id} onClick={() => !isLocked && setSelectedCourse(course.id)}
                  className="w-full rounded-[18px] text-left belly-press belly-card-shadow"
                  ...
                  <div className="flex items-start gap-3 p-[14px_16px_10px]">
                    ...
                    <span className="text-[12px] font-bold shrink-0 mt-2" style={{ color: "#D4906A" }}>
                      {isLocked ? <span className="text-[10px] px-2 py-1 rounded-full" style={{ background: "#FFF4EE", border: "1px solid #FFCDB4", color: "#D4906A" }}>🔒 Premium</span>
                  ...
                  <div className="flex flex-wrap gap-1.5 px-4 pb-3">
                    {course.tags.map(tag => (
                      <span key={tag} className="text-[10px] px-2 py-0.5 rounded-full belly-badge-glass" style={{ background: "#FFF4EE", border: "1px solid #FFCDB4", color: "#D4906A" }}>{tag}</span>
                    ))}
                  </div>
                </button>
              );
      ...
      {cartCount > 0 && tab === "remedies" && (
        <button onClick={() => setShowCart(true)}
          className="fixed bottom-20 right-4 w-[52px] h-[52px] rounded-full flex items-center justify-center belly-btn-press z-40"
          style={{ background: "#FFB899", boxShadow: "0 4px 12px rgba(42,18,0,0.15)" }}>
      ...
      {showCart && (
        <div className="fixed inset-0 bg-black/40 z-[200] flex items-end" onClick={() => setShowCart(false)}>
          <div className="bg-white w-full rounded-t-[24px] max-h-[70vh] flex flex-col belly-slide-up" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 pt-4 pb-3 shrink-0" style={{ borderBottom: "1px solid #FFE4D4" }}>
              ...
              {cart.map(item => (
                <div key={item.product.id} className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => updateQty(item.product.id, -1)} className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: "#FFF0E8" }}><Minus size={12} style={{ color: "#D4906A" }} /></button>
                    <span className="text-[13px] font-semibold w-5 text-center" style={{ color: "#2A1200" }}>{item.qty}</span>
                    <button onClick={() => updateQty(item.product.id, 1)} className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: "#FFF0E8" }}><Plus size={12} style={{ color: "#D4906A" }} /></button>
                  </div>
                </div>
              ))}
            </div>
            <div className="shrink-0 px-5 pb-6 pt-3" style={{ borderTop: "1px solid #FFE4D4" }}>
              ...
              <button onClick={placeOrder} disabled={ordering}
                className="w-full h-12 rounded-[14px] text-[14px] font-bold belly-btn-press disabled:opacity-50" style={{ background: "#FFB899", color: "#2A1200" }}>
